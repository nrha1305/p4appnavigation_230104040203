...
fun HubScreen(nav: NavHostController, tab: HubTab) {
    Scaffold(
        ...
        when (tab) {
            ...
            is HubTab.Messages -> MessagesTab(
                Modifier.padding(padding)
            ) { id ->
                nav.navigate(Route.HubMsgDetail.of(id))
            }
...
private fun DashboardTab(mod: Modifier = Modifier) {
    Column(mod.fillMaxSize().padding(16.dp), verticalArrangement = Arrangement.spacedBy(16.dp)) {
        ...
        InfoCard(
            "Welcome!",
            listOf(
                "This screen represents a dashboard within one Activity, aggregating key metrics, navigation shortcuts, and recent updates, enabling quick access to core features without switching contexts or launching additional activities.",
                "Layar ini merepresentasikan dasbor dalam satu Activity, yang menggabungkan metrik utama, pintasan navigasi, dan pembaruan terbaru, sehingga memungkinkan akses cepat ke fitur inti tanpa beralih konteks atau meluncurkan Activity lain."
            )
        )
    }
}
    ...
    Column(mod.fillMaxSize().padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        ...
        ElevatedCard(
            ...
            ListItem(
                ...
                supportingContent = { Text("Use Scaffold and NavigationBar for app structure.") },
